...
tftp-client.
Usage:
  tftp-client.py <action> <filename> [[-s | -b ] --mode=<mode>]
...
def send_rq(filename, action, mode):
    """
...
def send_data(filename, ack, server):
...
def main():
    ...
    # Send request
    send_rq(filename, arguments['<action>'], mode)
